...
import { deleteOrganization, listOrganizationPagination } from 'src/main/services/OrganizationalManagement'
...
const OrganizationList = (props: Props) => {
  ...
  const fetchAllOrganization = useCallback(async (page: number): Promise<void> => {
    setLoading(true)
    try {
      const response = await listOrganizationPagination(page, organizationName ?? '')
  ...
  const handlePageChange = (page: any) => {
    fetchAllOrganization(page)
  }

  useEffect(() => {
    fetchAllOrganization(currentPage)
  }, [fetchAllOrganization])

  async function onDelete(id: string) {
    try {
      await deleteOrganization(id)
      await fetchAllOrganization(currentPage)
    } catch (err) {
      console.log(err)
    }
  }
  ...
  /**
   * confirmDelete
   * handle give alert when organization want to be deleted and hit API to delete organization
   * @param row
   */
  ...
  return (
    <div>
      ...
      <Paper sx={{ width: '100%', overflow: 'hidden' }}>
        {DisplayDataTable(columns, allOrganization, loading, totalRow, currentPage, handlePageChange)}
      </Paper>
    </div>
  )
}
